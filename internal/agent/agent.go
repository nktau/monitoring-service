...
func (mem *memStorage) Start(serverUrl string, reportInterval, pollInterval int) {
	go mem.GetRuntimeMetrics(pollInterval)
	for {
		go mem.SendRuntimeMetric(serverUrl, reportInterval)
	}
	//for {
	//	if mem.counter%5 == 0 {
...
func (mem *memStorage) SendRuntimeMetric(serverURL string, reportInterval int) {
...
func (mem *memStorage) GetRuntimeMetrics(pollInterval int) {
	for {
